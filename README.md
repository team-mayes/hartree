...
Once you have Maven and cmayes-common installed, you can build Hartree.
        ...
        $ cd hartree
        $ git checkout hartree-1.2.4
        $ mvn clean package

Note that you should check out the tag that corresponds to the release that you wish to build.

The artifact at `hartree-cli/target/hartree-cli-1.2.4.jar` (or whichever version you've built)
is what you'll use.
...
The basic usage is:
	...
	$ java -jar hartree-cli-1.2.4.jar
...
Let's try the normal mode option: 
	...
	$ java -jar hartree-cli-1.2.4.jar norm
...
As the error message indicate, We need to specify a source to process.  Let's do a single file first.
	...
	$ java -jar hartree-cli-1.2.4.jar norm -f glucose5m062xEtOHnorm.log
...
file, but it's just as easy to point Hartree to an output directory:
	...
	$ java -jar hartree-cli-1.2.4.jar norm -f glucose5m062xEtOHnorm.log -o out
...
Now, let's try processing a whole directory at once:
	...
	$ java -jar hartree-cli-1.2.4.jar norm -d ~/g09/ -o out
